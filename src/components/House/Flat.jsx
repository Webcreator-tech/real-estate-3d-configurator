    ...
    >
      ...
      {type === "sofa" && (
        <group>
          ...
          {[-0.85, 0.85].map((x) =>
            [-0.3, 0.3].map((z) => (
              <mesh key={`leg_${x}_${z}`} position={[x, 0.05, z]} castShadow>
      ...
      {type === "dining_table" && (
        <group>
          ...
          {[-0.75, 0.75].map((x) =>
            [-0.4, 0.4].map((z) => (
              <mesh key={`tbl_leg_${x}_${z}`} position={[x, 0.35, z]} castShadow>
                <cylinderGeometry args={[0.03, 0.025, 0.7, 16]} />
          ...
          {/* Dining Chairs */}
          {[-0.45, 0.45].map((x) => (
            <group key={`chair_front_${x}`} position={[x, 0, 0.65]}>
              ...
              {[-0.18, 0.18].map((cx) =>
                [-0.18, 0.18].map((cz) => (
                  <mesh key={`chair_leg_${cx}_${cz}`} position={[cx, 0.22, cz]} castShadow>
          ...
          {[-0.45, 0.45].map((x) => (
            <group key={`chair_back_${x}`} position={[x, 0, -0.65]} rotation={[0, Math.PI, 0]}>
              <mesh position={[0, 0.45, 0]} castShadow receiveShadow>
                <boxGeometry args={[0.42, 0.04, 0.42]} />
                <meshStandardMaterial color={color} roughness={0.7} />
              </mesh>
              ...
              {[-0.18, 0.18].map((cx) =>
                [-0.18, 0.18].map((cz) => (
                  <mesh key={`chair_leg_b_${cx}_${cz}`} position={[cx, 0.22, cz]} castShadow>
                    <cylinderGeometry args={[0.015, 0.012, 0.44, 12]} />
                    <meshStandardMaterial color="#1a1a1a" roughness={0.4} />
                  </mesh>
                ))
      ...
      {type === "bed" && (
        <group>
          ...
          {/* Headboard */}
          <mesh position={[0, 0.75, -1.0]} castShadow receiveShadow>
            <boxGeometry args={[1.9, 0.9, 0.12]} />
            <meshStandardMaterial color="#2d2d2d" roughness={0.6} />
          </mesh>
          {/* Pillows */}
          {[-0.45, 0.45].map((x) => (
            <mesh key={`pillow_${x}`} position={[x, 0.58, -0.65]} rotation={[-0.2, 0, 0]} castShadow>
              <boxGeometry args={[0.6, 0.12, 0.4]} />
              <meshStandardMaterial color="#ffffff" roughness={0.9} />
            </mesh>
          ))}
          {/* Blanket / Duvet */}
          <mesh position={[0, 0.56, 0.25]} castShadow receiveShadow>
      ...
      {type === "coffee_table" && (
        <group>
          ...
          {[0, (2 * Math.PI) / 3, (4 * Math.PI) / 3].map((angle, idx) => {
            ...
            return (
              <mesh key={`ctl_${idx}`} position={[legX, 0.18, legZ]} castShadow>
                <cylinderGeometry args={[0.018, 0.014, 0.36, 16]} />
      ...
      {type === "armchair" && (
        <group>
          ...
          {/* Arms */}
          {[-0.35, 0.35].map((x) => (
            <mesh key={`arm_${x}`} position={[x, 0.5, 0]} castShadow receiveShadow>
          ...
          {[-0.28, 0.28].map((x) =>
            [-0.28, 0.28].map((z) => (
              <mesh key={`ac_leg_${x}_${z}`} position={[x, 0.1, z]} castShadow>
                <cylinderGeometry args={[0.02, 0.015, 0.2, 16]} />
      ...
      {type === "indoor_plant" && (
        <group>
          ...
          <mesh position={[0, 0.22, 0]} castShadow receiveShadow>
            <cylinderGeometry args={[0.22, 0.16, 0.44, 24]} />
          ...
          {[0, 1, 2, 3, 4].map((i) => {
            ...
            const lx = Math.sin(leafAngle) * 0.25;
            const lz = Math.cos(leafAngle) * 0.25;
            return (
              <group key={`plant_leaf_${i}`} position={[0, 0.44, 0]} rotation={[0.2, leafAngle, 0]}>
                <mesh position={[lx, 0.35 + i * 0.08, lz]} rotation={[0.3, 0, 0.2]} castShadow>
...
/**
 * Flat Component
 * Loads /models/flat.glb, prepares individual wall materials, and applies shared customization.
 */
export default function Flat() {
  ...
  // Clone scene once to avoid mutating the cached GLTF directly
  const { clonedScene, originalMaterials } = useMemo(() => {
    ...
    clone.traverse((child) => {
      if (child.isMesh) {
        ...
        // Clone material instance per mesh so each wall can be independently styled
        if (child.material) {
          origMap.set(child.name, child.material.clone());
          child.material = child.material.clone();
        }

        // Tag wall meshes for identification
        const isWall =
    ...
    return { clonedScene: clone, originalMaterials: origMap };
  }, [scene]);

  // Dynamically apply wall customization colors & selection highlights
  useEffect(() => {
    clonedScene.traverse((child) => {
      if (child.isMesh && child.userData.isWall && child.material) {
        const customColor = wallColors[child.name] || wallColors.__all__;
        const originalMat = originalMaterials.get(child.name);

        if (customColor) {
          child.material.color.set(customColor.hex || customColor.color || "#ffffff");
          ...
          if (wallFinish === "gloss") {
            ...
            child.material.roughness = customColor.roughness ?? 0.85;
            child.material.metalness = customColor.metalness ?? 0.0;
          }
        } else if (originalMat) {
          child.material.color.copy(originalMat.color);
          child.material.roughness = originalMat.roughness;
          child.material.metalness = originalMat.metalness;
        }

        // Subtle highlight when the wall is selected by user
  ...
  }, [clonedScene, originalMaterials, wallColors, selectedWall, wallFinish]);

  // Handle pointer down / click to select walls
  const handlePointerDown = (e) => {
    // Only intercept if clicked mesh is a wall
    if (e.object && e.object.userData.isWall) {
      e.stopPropagation();
      selectWall(e.object.name);
    }
  };
