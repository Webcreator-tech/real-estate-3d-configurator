...
export default function CustomizationToolbar() {
  ...
  return (
    <nav className="customization-toolbar" aria-label="Customization toolbar">
      <div className="toolbar-container">
        {/* 1. Walls */}
        <button
          className={`toolbar-btn ${activePanel === "walls" ? "active" : ""}`}
        ...
        >
          ...
          {selectedWall && <span className="toolbar-status-dot" />}
        </button>

        {/* 2. Furniture */}
        <button
          className={`toolbar-btn ${activePanel === "furniture" ? "active" : ""}`}
        ...
        >
          ...
          {furniture.length > 0 && (
            <span className="toolbar-badge">{furniture.length}</span>
          )}
        </button>

        {/* 3. Lighting */}
        <button
          className={`toolbar-btn ${activePanel === "lighting" ? "active" : ""}`}
        ...
        {/* 4. 360° Orbit / Walkthrough Mode Toggle */}
        <button
          className={`toolbar-btn mode-switch ${mode === "orbit360" ? "active orbit-active" : ""}`}
          onClick={handleModeToggle}
          title={mode === "orbit360" ? "Return to Walkthrough" : "Switch to 360° Orbit View"}
        >
          <span className="toolbar-icon">
            {mode === "orbit360" ? (
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
